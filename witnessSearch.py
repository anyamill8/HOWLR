###################################################################################
# Given an IP address, convert to IP /24 prefix and search using Censys database
# to enumerate avaliable ports. Search each port for services and request SSL/TLS
# infromation from each. Returns total number of results and port/service distribution
# as well IP addresses that have a direct and verifyable certificate within the IP 
# prefix. Can be used to find only verifiable certificates (verification on) or can 
# be used to find self-signed or invalid domain-name certificates (verification off).
# If a certificate is not direclty found by using the IP address, the IP address will
# be resolved to a domain name and this domain name will be used to prompt for a 
# certificate. Self-signed certificates are coined "indirect certificates" and 
# certificates that require domain name resolution are coined "reverseDNS" certificates.
...
db = client["backup"] # Can change name of database here

# Resolves IP address to registered domain name
def reverse_dns_lookup(ip):
    try:
        hostname = socket.gethostbyaddr(ip)[0]
        print(f"Reverse DNS for {ip} resolved to {hostname}")
        return hostname
    except socket.herror as e:
        print(f"Reverse DNS lookup failed for {ip}: {e}")
        return None

# Converts IPv4 address to /24 prefix
def ip_to_prefix(ip):
...
# Extract live certificate details (subject, issuer, public key, fingerprint, expiration) into a dictionary
...
## Fetch certificate information for a given IP and port using a multi-step approach:
    ## 1. Try to get a direct certificate.
    ## 2. If not found, attempt to fetch an indirect certificate.
    ## 3. Perform a reverse DNS lookup to fetch a reverse DNS certificate.
def fetch_live_certificate(ip, port, censys_certificate=None):
    try:
        # Step 1: Attempt direct certificate retrieval
        ...
        with socket.create_connection((ip, port), timeout=5) as sock:
            with context.wrap_socket(sock, server_hostname=None) as ssock:
                ...
                if cert_der:
                    ...
                    # gather live certificate details
                    live_cert_details = extract_cert_details(cert)

                    # check to see if Censys certificate info matches
                    labels = []
                    ...
                    # do not complete step 2 and 3 if 1 is successful, return
                    return {
                        "type": "direct_cert",
                        "certificate": live_cert_details
                    }
    except Exception as e:
        print(f"No direct certificate found for {ip}:{port}. Proceeding to indirect certificate. Error: {e}")

    # Step 2: Attempt indirect certificate retrieval which corresponds to gateway/load balancer/self-signed cert
    indirect_cert = None
    try:
        context = ssl.create_default_context()
        # disable both hostname and verification as cert may not have proper CA issuer
        context.check_hostname = False
        context.verify_mode = ssl.CERT_NONE

        # Fetch indirect certificate 
        with socket.create_connection((ip, port), timeout=5) as sock:
            with context.wrap_socket(sock, server_hostname=None) as ssock:
                cert_der = ssock.getpeercert(binary_form=True)
                if cert_der:
                    ...
                    # Add labels to the live_cert_details if mismatch
                    live_cert_details["labels"] = labels

                    # Store info and attempt Step 3
                    indirect_cert = {
                        "type": "indirect_cert",
                        "certificate": live_cert_details
                    }
    except Exception as e:
        print(f"No indirect certificate found for {ip}:{port}. Error: {e}")

    # Step 3: Attempt reverse DNS lookup and certificate retrieval
    reverse_dns_cert = None
    try:
        # Try to resolve IP to domain name
        hostname = reverse_dns_lookup(ip)
        if hostname:
            context = ssl.create_default_context()
            context.check_hostname = False  # Bypass hostname verification as reverse DNS will not match typical domain name
            context.verify_mode = ssl.CERT_OPTIONAL # Do not require verification as cert may not have proper CA issuer
            ...
            with socket.create_connection((ip, port), timeout=5) as sock:
                with context.wrap_socket(sock, server_hostname=hostname) as ssock:
                    ...
                    if cert_der:
                        ...
                        # Save information
                        reverse_dns_cert = {
                            "type": "reverse_DNS_cert",
                            "certificate": extract_cert_details(cert),
                            "reverse_dns": hostname
                        }
    except Exception as e:
        print(f"No reverse DNS certificate found for {ip}:{port}. Error: {e}")

    # Return both indirect and reverse DNS cert information if direct cert failed
    if indirect_cert or reverse_dns_cert:
        return {
            "indirect_cert": indirect_cert,
            "reverse_DNS_cert": reverse_dns_cert
        }
...
def fetch_services(ip):
    try:
        ...
        # collect port number, service name, transport protocal, censys, and live certificate for
        # each service on IP
        for service in services_data:
            if isinstance(service, dict):
                ...
                # Fetch live certificate info for each port
                live_certificate_info = fetch_live_certificate(ip, port, censys_certificate_info)

                # Track valid certificate if live_certificate_info was successfully retrieved
                # Helpful for debugging
                if live_certificate_info:
                    print(f"Certificate found for {ip}:{port}")
...
def search_subnet(ip):
    ...
    collection_name = network_prefix.replace("/", "_").replace(".", "_")
    collection = db[collection_name]

    # Create a collection for final statistics
    stats_collection = db["rand_statistics"]
    ...
    ip_with_direct_cert = 0  # Tracks IPs with at least one direct cert
    direct_cert_ips = []  # Stores IPs with at least one direct cert
    ip_with_indirect_cert = 0 # Tracks IPs with at least one indirect cert
    ip_with_reverseDNS_cert = 0 # Tracks IPs with at least one reverse DNS cert
    ip_with_indirect_and_reverse_dns = 0  # Tracks IPs with indirect cert and reverse DNS cert but no direct cert
    ...
    for ip in network: # includes IPs 0-255
        ...
        # initialize IP's specific certificate type
        has_direct_cert = False
        has_indirect_cert = False
        has_reverseDNS_cert = False
        has_reverse_indirect_cert = False
        ...
        if ip_data and ip_data["services"]: 
            ...
            for service in ip_data["services"]:
                ...
                if live_certificate:
                    ...
                    if live_certificate.get("indirect_cert"):
                            has_indirect_cert = True
                    if live_certificate.get("reverse_DNS_cert"):
                            has_reverseDNS_cert = True
                    if live_certificate.get("indirect_cert") and live_certificate.get("reverse_DNS_cert"):
                            has_reverse_indirect_cert = True

            # Increment counters for specific certificate types
            if has_direct_cert:
                ip_with_direct_cert += 1
                direct_cert_ips.append(str(ip))  # Add IP to the list of direct_cert IPs
            if has_indirect_cert: 
                ip_with_indirect_cert += 1
            if has_reverseDNS_cert:
                ip_with_reverseDNS_cert += 1
            if has_reverse_indirect_cert:
                ip_with_indirect_and_reverse_dns += 1
    ...
    # print count of IPs with indirect and reverse_DNS cert
    print(f"Total IP addresses with no direct_cert but with both indirect_cert and reverse_DNS_cert: {ip_with_indirect_and_reverse_dns}")

    # print count of IPs with indirect cert
    print(f"Total IP addresses with indirect_cert: {ip_with_indirect_cert}")

    # print count of IPs with reverse_DNS cert
    print(f"Total IP addresses with reverse_DNS_cert: {ip_with_reverseDNS_cert}")
    ...
    run_stats = {
        ...
        "ip_with_indirect_and_reverse_dns": ip_with_indirect_and_reverse_dns,
        "ip_with_indirect_cert": ip_with_indirect_cert,
        "ip_with_reverseDNS_cert": ip_with_reverseDNS_cert,
