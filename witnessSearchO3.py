...
# enumerate avaliable ports. Search each port except port 80, beginning with port 443
    ...
    try:
        ...
        for service in all_services_to_process:
            if isinstance(service, dict):
                ...
                # Ignore port 80 when searching for certificates
                if not live_cert_found and port != 80:
...
def search_subnet(ip):
    ...
    collection_name = f"{network_prefix.replace("/", "_").replace(".", "_")}o3"
