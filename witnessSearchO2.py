...
# to enumerate avaliable ports. Search each port, beginning with port 443
# for services and request SSL/TLS infromation from each. Quit searching ports once 
# a single certificate is found # for a single IP. Returns total number of results 
# and port/service distribution  as well IP addresses that have a direct and 
# verifyable certificate within the IP prefix. Do not attemt to collect "indirect" 
# or "reverse DNS" certificates.
...
def fetch_services(ip):
    try:
        ...
        # Flag to indicate if a live certificate has been found
        live_cert_found = False
        ...
        for service in all_services_to_process:
            if isinstance(service, dict):
                ...
                # Fetch live certificate info only if not already found
                if not live_cert_found:
                    live_certificate_info = fetch_live_certificate(ip, port, censys_certificate_info)
                    if live_certificate_info:
                        live_cert_found = True
                else:
                    live_certificate_info = None
...
def search_subnet(ip):
    ...
    collection_name = f"{network_prefix.replace("/", "_").replace(".", "_")}o2"
