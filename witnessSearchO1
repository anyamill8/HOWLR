...
# prefix. Do not attemt to collect "indirect" or "reverse DNS" certificates.
...
db = client["PLACE_NAME_HERE"] # Can change name of database here
...
## Fetch certificate information for a given IP and port (direct only)
            ...
            with context.wrap_socket(sock, server_hostname=None) as ssock:
                ...
                if cert_der:
                    ...
                    if cert.not_valid_before_utc > current_time or cert.not_valid_after_utc < current_time:
                        ...
                        return None
                    # Extract live certificate details
                    ...
                    # Add labels to the live_cert_details if therre is a mismatch
...
def search_subnet(ip):
    ...
    collection_name = f"{network_prefix.replace("/", "_").replace(".", "_")}o1"
